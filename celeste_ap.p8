...
selected_respawn_option=1
load_game_text = 0
deathlink_enabled=false
deaths_since_last_deathlink=0
deaths_required_for_deathlink=10
deathlinked=false
...
locs = {
	...
	["x4y3"]=		{3, 2^4},
	["x5y3"]=		{3, 2^5}, -- 3000m
	["victory"]=	{3, 2^6}, -- 3100m (summit)
	["x0y0s"]=		{4, 2^0},
	["x2y0s"]=		{4, 2^1},
	["x3y0s"]=		{4, 2^2},
	["x4y0s"]=		{4, 2^3}, -- 500m strawberry
	["x6y0s"]=		{4, 2^4},
	["x0y1s"]=		{4, 2^5},
	["x3y1s"]=		{4, 2^6}, -- old site strawberry
	["x4y1s"]=		{4, 2^7},
	["x5y1s"]=		{5, 2^0},
	["x6y1s"]=		{5, 2^1}, -- 1500m strawberry
	["x0y2s"]=		{5, 2^2},
	["x2y2s"]=		{5, 2^3},
	["x6y2s"]=		{5, 2^4},
	["x0y3s"]=		{5, 2^5}, -- 2500m strawberry
	["x1y3s"]=		{5, 2^6},
	["x3y3s"]=		{5, 2^7},
	["x4y3s"]=		{6, 2^0},
	["x5y3s"]=		{6, 2^1},
...
items = {
	["springs"]=			{7, 2^0}, 
	["balloons"]=			{7, 2^1}, 
	["crumbling blocks"]=	{7, 2^2}, 
	["breakable blocks"]=	{7, 2^3}, 
	["white clouds"]=		{7, 2^4}, 
	["keys"]=				{7, 2^5}, 
	["double dash"]=		{7, 2^6}, 
	["500m checkpoint"]=	{8, 2^0}, 
	["1000m checkpoint"]=	{8, 2^1},
	["1500m checkpoint"]=	{8, 2^2},
	["2000m checkpoint"]=	{8, 2^3},
	["2500m checkpoint"]=	{8, 2^4},
	["old site checkpoint"]={8, 2^5},
	["2200m checkpoint"]=	{8, 2^6},
}
...
function debug_unlock_all_respawns()
	for i,v in pairs(respawn_table) do
		v.unlocked=true
	end
end
...
function check_location(id)
	-- the game will attempt to check non-existent title and summit room locations. just ignore it
	if (locs[id] == nil) return
	gpio_adr = 0x5f80 + locs[id][1]
	bank = peek(gpio_adr)
	bank = bank | locs[id][2]
	poke(gpio_adr, bank)
end
...
function give_items()
	for key,item in pairs(items) do
		...
		--item = items[key]
		gpio_adr = 0x5f80 + item[1]
		bank = peek(gpio_adr)
		if (bank & item[2]) ~= 0 then
			give_vanilla_item(key)
			items[key] = nil
			--remove the item so we cannot recieve it again
		end
	end

	strawbs_have = peek(0x5f80+38)
end

function give_vanilla_item(typ)
	if typ == "springs" then
		spring.locked=false
	end
	if typ == "balloons" then
		balloon.locked=false
	end
	if typ == "crumbling blocks" then
		fall_floor.locked=false
	end
	if typ == "breakable blocks" then
		fake_wall.locked=false
	end
	if typ == "white clouds" then
		platform.locked=false
	end
	if typ == "keys" then
		key.locked=false
	end
	if typ == "double dash" then
		can_double_dash=true
	end
	if typ == "500m checkpoint" then
		respawn_table[2].unlocked=true
	end
	if typ == "1000m checkpoint" then
		respawn_table[3].unlocked=true
	end
	if typ == "old site checkpoint" then
		respawn_table[4].unlocked=true
	end
	if typ == "1500m checkpoint" then
		respawn_table[5].unlocked=true
	end
	if typ == "2000m checkpoint" then
		respawn_table[6].unlocked=true
	end
	if typ == "2200m checkpoint" then
		respawn_table[7].unlocked=true
	end
	if typ == "2500m checkpoint" then
		respawn_table[8].unlocked=true
	end
end

function check_settings()
	local gpio_adr = 0x5f80+40
	local settings = peek(gpio_adr)
	if (settings & 2^0) != 0 then
		deathlink_enabled = true
	end

	strawbs_need = peek(0x5f80+39)
	deaths_required_for_deathlink = peek(0x5f80+41)
	-- failsafe. if this happens, i know something went terribly wrong, since 99 amnesty is not normally possible
	if (deaths_required_for_deathlink == 0) deaths_required_for_deathlink = 99
end

function check_deathlink()
	if (not deathlink_enabled) return
	local gpio_adr = 0x5f80+42
	if peek(gpio_adr) > 0 then
		poke(gpio_adr,0)
		kill_me_please = true
		deathlinked = true
		deaths_since_last_deathlink=0
...
function _init()
	menuitem(1, "retry", function() kill_me_please = true end)
	menuitem(2, "goto: ⬅️100m➡️", function(b)
		if b&1>0 then
			repeat
				selected_respawn_option -= 1
				if (selected_respawn_option < 1) selected_respawn_option = #respawn_table
			until respawn_table[selected_respawn_option].unlocked != false
		elseif b&2>0 then
			repeat
				selected_respawn_option += 1
				if (selected_respawn_option > #respawn_table) selected_respawn_option = 1
			until respawn_table[selected_respawn_option].unlocked != false
		else
			if is_title() then
				load_game_text = 60
			...
			-- warping to another checkpoint while opening the big chest softlocks the game
			if (pause_player) return
			local spawnpoint = respawn_table[selected_respawn_option]
			music(spawnpoint.music,0,7)
			load_room(spawnpoint.x,spawnpoint.y)
			if spawnpoint.name == "2500m" then
				max_djump=2
				new_bg=true
			else
				max_djump=1
		...
		menuitem(2, "goto: ⬅️"..respawn_table[selected_respawn_option].name.."➡️")
	end)
	title_screen()
end
...
player = 
{
	...
	init=function(this) 
		local max_allowed_jump = can_double_dash and max_djump or 1
		this.p_jump=false
		this.p_dash=false
		this.grace=0
		this.jbuffer=0
		this.djump=max_allowed_jump
		this.dash_time=0
	...
	update=function(this)
		...
		if kill_me_please then
			kill_player(this)
			kill_me_please = false end
		...
		if on_ground then
			this.grace=6
			local max_allowed_jump = can_double_dash and max_djump or 1
			if this.djump<max_allowed_jump then
			 psfx(54)
			 this.djump=max_allowed_jump
			end
...
balloon = {
	...
	update=function(this) 
		if this.spr==22 then
			...
			local hit = this.collide(player,0,0)
			local max_allowed_jump = can_double_dash and max_djump or 1
			if hit~=nil and hit.djump<max_allowed_jump then
				psfx(6)
				init_object(smoke,this.x,this.y)
				hit.djump=max_allowed_jump
				this.spr=0
	 ...
	 local max_allowed_jump = can_double_dash and max_djump or 1
		if hit~=nil then
		 hit.djump=max_allowed_jump
			sfx_timer=20
			sfx(13)
			check_location("x"..room.x.."y"..room.y.."s")
	...
	draw=function(this)
		...
		if this.spd.y==0 and hit~=nil then
		 music_timer=45
			...
			destroy_object(this)
			max_djump=2
			hit.djump=can_double_dash and 2 or 1
		end
...
flag = {
	...
	draw=function(this)
		...
		if this.show then
			...
			spr(26,55,6)
			print("x"..strawbs_have.."/"..strawbs_need,64,9,7)
			draw_time(49,16)
			print("deaths:"..deaths,48,24,7)
		elseif this.check(player,0,0) then
			sfx(55)
			if (strawbs_have>=strawbs_need) check_location("victory")
	  sfx_timer=30
			this.show=true
		end
	end
...
room_title = {
	...
	draw=function(this)
		...
		elseif this.delay<0 then
			...
			draw_time(4,4)
			if (deathlink_enabled) draw_deaths(4,10)
		end
	end
}
...
function kill_player(obj)
	...
	if deathlinked then
		...
		deaths_since_last_deathlink+=1
	end
	if deaths_since_last_deathlink >= deaths_required_for_deathlink and deathlink_enabled then
		deaths_since_last_deathlink = 0
		send_deathlink()
	end
	shake=10
...
function load_room(x,y)
	...
	for tx=0,15 do
		for ty=0,15 do
			...
			if tile==11 and not platform.locked then
				init_object(platform,tx*8,ty*8).dir=-1
			elseif tile==12 and not platform.locked then
				init_object(platform,tx*8,ty*8).dir=1
			else
				foreach(types, 
				function(type) 
					if type.tile == tile and not type.locked then
...
function draw_deaths(x,y)
	local drawcol = (deaths_required_for_deathlink - deaths_since_last_deathlink == 1) and 8 or 7
	
	rectfill(x,y,x+52,y+6,0)
	print("deaths: "..deaths_since_last_deathlink.."/"..deaths_required_for_deathlink,x+1,y+1,drawcol)
